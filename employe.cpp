...
bool Employe::ajouter() {
    QSqlQuery query;
    query.prepare("INSERT INTO employe ( nom, prenom, role, salaire,email) "
            //      "VALUES (seq_employe.NEXTVAL, :nom, :prenom, :role, :salaire,:email)");
                  "VALUES ( :nom, :prenom, :role, :salaire,:email)");
...
bool Employe::modifier() {
    ...
    // Prepare the SQL query with proper spacing
    query.prepare("UPDATE employe SET nom = :nom, prenom = :prenom, role = :role, "
...
QSqlQueryModel* Employe::afficherEmpFeedback(int id) {
    QSqlQueryModel* model = new QSqlQueryModel();
    model->setQuery("SELECT  points FROM EMPLOYE WHERE ID_E=" + QString::number(id));
    return model;
}
