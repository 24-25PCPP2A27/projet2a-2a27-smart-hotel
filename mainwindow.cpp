...
void MainWindow::on_ajouter_f_2_clicked()
{
    ...
    query.prepare("INSERT INTO feedback ( id_employe, feedback) "
                      "VALUES ( :id_employe, :feedback)");
...
void MainWindow::on_ajouter_f_2_clicked()
{
    ...
    if (query.exec()) {
        ...
        QSqlQuery updateQuery;
        updateQuery.prepare("MERGE INTO EMPLOYE USING DUAL "
                            "ON (ID_E = :id_employe) "
                            "WHEN MATCHED THEN "
                            "    UPDATE SET points = points + :points "
                            "WHEN NOT MATCHED THEN "
                            "    INSERT (ID_E,points) VALUES (:id_employe, :points) ");
        updateQuery.bindValue(":id_employe", id_employe);
        updateQuery.bindValue(":points", points_change);

        if (updateQuery.exec()) {
            QMessageBox::information(this, "Succès", "Feedback ajouté avec succès !");
            ui->aff_feed_2->setModel(e.afficherEmpFeedback(id_employe));
...
void MainWindow::on_email_2_cursorPositionChanged(int arg1, int arg2)
{
...
void MainWindow::on_groupBox_4_clicked()
{
